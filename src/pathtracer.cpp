...
Spectrum PathTracer::estimate_direct_lighting(const Ray& r, const Intersection& isect) {
  ...
  for (SceneLight *sl : scene->lights) {
    ...
    if (sl->is_delta_light()) {
      ...
      num_samps = ns_area_light;
    }
    //sample it now
    for (int i = 0; i < num_samps; i++) {
      Vector3D wi;
      float distToLight, pdf;
      Spectrum light = sl->sample_L(hit_p, &wi, &distToLight, &pdf);
      Vector3D w_in = w2o * wi;
      if (w_in.z < 0) {
        continue;
      } //this sample lies beyond surface
      Vector3D shadowOrg = hit_p + (wi*EPS_D);
      Vector3D shadowDir = wi;
      Ray shadow = Ray(shadowOrg, shadowDir);
      shadow.max_t = distToLight;
      Intersection shadowBVH;
      bool intersects = bvh->intersect(shadow, &shadowBVH);
      if (!intersects) {
//        bsdf myBsdf = isect.primtiive->get_bsdf();
        Spectrum irradiance = light * w_in.z * isect.primitive->get_bsdf()->f(w_out, w_in);
        L_out += (irradiance)/(pdf);
      }
    }
    L_final += L_out/num_samps;
    L_out = Spectrum(0.0,0.0,0.0); 
  }
  return L_final;
...
Spectrum PathTracer::estimate_indirect_lighting(const Ray& r, const Intersection& isect) {
 ...
 // printf("indirect light");
  ...
  Vector3D w_in;
  float pdf;
  Spectrum sample = isect.primitive->get_bsdf()->sample_f(w_out, &w_in, &pdf);
  //now see if you must terminate the ray?
  double scaledReflectance = sample.illum() * 10.0; //russ roul probability
  double terminateProbability = 1 - clamp(scaledReflectance, 0.0f, 1.0f);
  bool terminate = coin_flip(terminateProbability);
  if(!terminate) {
    Vector3D reflectOrigin = hit_p + EPS_D*(o2w*w_in);
    Vector3D reflectDirection = o2w*w_in;
    Ray reflectRay = Ray(reflectOrigin, reflectDirection);
    reflectRay.depth = r.depth-1;
    Spectrum inRadiance = trace_ray(reflectRay, isect.bsdf->is_delta());
    Spectrum outRadiance = (inRadiance * sample * abs_cos_theta(w_in))/(pdf*(1-terminateProbability));
    return outRadiance;
  } else {
...
Spectrum PathTracer::trace_ray(const Ray &r, bool includeLe) {
 ...
 // return normal_shading(isect.n);
  ...
  if (!isect.bsdf->is_delta() /*&& (r.depth < max_ray_depth)*/)  {
 //   printf("direct");
...
//  int num_samples = ns_aa; // total samples to evaluate
  ...
  Spectrum s;
  if (num_samples == 1) {
    xNew = ((double)x + 0.5)/sampleBuffer.w;
    yNew = ((double)y + 0.5)/sampleBuffer.h;
    Ray r1 = camera->generate_ray(xNew, yNew);
    r1.depth = max_ray_depth;
    return trace_ray(r1, true);
  }
  //printf("numsamples=%d", num_samples);
  for (int i = 0; i < num_samples; i++) {
    Vector2D v = gridSampler->get_sample();
    xNew = ((double)x + v.x)/sampleBuffer.w;
    yNew = ((double)y + v.y)/sampleBuffer.h;
    //printf("xNew yNew= %f, %f\n", xNew, yNew);
    Ray r2 = camera->generate_ray(xNew, yNew);
    r2.depth = max_ray_depth; //part 4 russian roullete
    s += trace_ray(r2, true);
   // printf("rbg=%f, %f, %f\n", r, b, g);
    //r += s.r;
    //b += s.b;
  ...
  //Vector2D origin = Vector2D(x,y); // bottom left corner of the pixel
  //return Spectrum(r/(double)num_samples, b/(double)num_samples, g/(double)num_samples);
