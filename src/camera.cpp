...
Ray Camera::generate_ray(double x, double y) const {
  ...
  double camX = bottomLeft.x * (1-x) + x*topRight.x;
  double camY = bottomLeft.y * (1-y) + y*topRight.y;
  //printf("camx and y =%f, %f \n", camX, camY);
  Vector3D camCoord = Vector3D(camX, camY, -1);
  Vector3D dir = (c2w*camCoord);
  dir.normalize();
  Ray r = Ray(pos, dir); //origin and direction vectors
